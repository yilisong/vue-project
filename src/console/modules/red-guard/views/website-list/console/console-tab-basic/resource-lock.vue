<template lang="html">
  ...
  <el-card class="item-padding">
    <el-row :gutter="20">
      <el-col :span="3">
        <p class="item-name">关键资源锁</p>
      </el-col>
      <el-col :span="15" class="description">
        <p>可将某个URL／文件类型访问强制设置登陆账户密码，以及仅允许某个IP/UA/地址可访问，有效防止暴力破解后台等敏感URL</p>
      </el-col>
      <el-col :span="6">
    ...
    <el-row :gutter="20">
      <el-col :offset="3" :span="21" class="top-line">
        <i-button @click="addAnticcWhiteList">新增</i-button>
      </el-col>
    </el-row>
    <el-row :gutter="20">
      <el-col :span="3">
        <p class="item-name">新增规则</p>
      </el-col>
      <el-col :span="21" class="table-content">
        <el-table :data="antiCc.whiteList" border style="width: 100%" v-loading="loading">
          <el-table-column label="基础锁">
            ...
            <el-table-column prop="" label="匹配内容">
              <template scope="scope">
                <div v-if="scope.row.isEditing">
                  <el-input :class="{'is-error':validArray[scope.$index].validContent}" v-if="scope.row.value.type !== 'suffix'" v-model="scope.row.value.content" placeholder="请输入内容" @blur="verifyContent(scope,'content')"></el-input>
          ...
          <el-table-column label="启用用户名密码">
            ...
            <el-table-column prop="" label="重试次数">
              <template scope="scope">
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column prop="" label="添加例外">
            <el-table-column prop="" label="例外方式">
              <template scope="scope">
              </template>
            </el-table-column>
            <el-table-column prop="" label="例外内容">
              <template scope="scope">
              </template>
            </el-table-column>
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      tipContent: [
        ...
        'IP段可以用 - 隔开，如：192.168.3.3-192.168.6.66，支持 CIDR格式',
        '类型为URL时内容以“/”开头，为后缀名时多个后缀名用“|”隔开',
  ...
  methods: {
    verifyContent(scope, tag) {
      ...
      switch (tag) {
        case 'content':
          ...
          if (value.type === 'ip') {
            if (!RULE.ipReg.test(value.content)) {
              this.validArray[scope.$index].validContent = true
            } else {
              this.validArray[scope.$index].validContent = false
              isError = false
        ...
        result => {
          if (result.ok) {
            result.json().then(res => {
              if (res && res.status.code === 1 && res.data && res.data.anti_cc) {
    ...
    handleButtonCcSave(scope) {
      ...
      let url
      if (row.id) {
        url = `Web.Domain.DomainId.${this.domainId}.Settings.anti_cc.rules.${row.id}`
        this.fetchPut(url, params).then(
          result => {
            if (result.ok) {
              result.json().then(res => {
        ...
        url = `Web.Domain.DomainId.${this.domainId}.Settings.anti_cc.rules`
        this.fetchPost(url, params).then(
          result => {
            if (result.ok) {
              result.json().then(res => {
                this.windowMessage(res)
    ...
    handleConfDel(index, row) {
      this.popoverArray[index].flag = false
      const url = `Web.Domain.DomainId.${this.domainId}.Settings.anti_cc.rules.${row.id}`
